...
const Navbar = () => {
  ...
  const links = (
    <div className="lg:flex text-2xl">
      ...
      <li>
        <NavLink to="/cartoon">Cartoon Movies</NavLink>
      </li>
    </div>
  );
  ...
  return (
    <div
      className={`navbar bg-base-100 ${isdark ? 'bg-black' : ''} flex justify-between md:justify-between items-center my-8 sticky top-0 z-50 backdrop-blur-lg`}
    >
      {/* Navbar Start */}
      <div className="navbar-start">
        <div className="dropdown">
        ...
        <a className="btn btn-ghost text-xl">
          <span className="text-red-500">MARVEL</span> MOVIES
        </a>
      </div>
      {/* Navbar Center */}
      <div className="navbar-center hidden lg:flex">
        ...
        {user && user.email ? (
          <div className="relative flex items-center gap-4">
            <div
              ...
              className="theme bg-white border p-1 rounded-full text-xl cursor-pointer"
            >
              {isdark ? <CiDark /> : <MdOutlineLightMode />}
            </div>
            <div className="group relative">
            ...
            <button
              className="btn bg-white text-yellow-500"
              onClick={logOut}
            >
              Log-Out
            </button>
          ...
          <div className="md:flex-col">
            <Link className="btn bg-white text-green-500" to="/login">
              Login
            </Link>
            <Link className="btn bg-white text-green-500" to="/register">
              Register
            </Link>
          </div>
        )}
